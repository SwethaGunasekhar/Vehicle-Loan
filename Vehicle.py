...
with open(r"D:\Data Science\Python\Code\VehicleLoan_Classification_Model.pkl",'rb') as f:
    model = pickle.load(f)
...
if select == "Home":
    st.write(":green[A non-banking financial institution (NBFI) is a type of financial institution that is not authorized to operate as a bank or is not under the supervision of a banking regulatory agency at the national or international level]")
    st.write(":green[The objective is to predict whether a client is likely to default on their vehicle loan payment or not.]")
    st.write("")
    img= Image.open(r"D:\Data Science\Python\Projects\Final Project\Vehicle Loan\toy-car-going-up-increasing-stack-coins-against-white-background.jpg")
